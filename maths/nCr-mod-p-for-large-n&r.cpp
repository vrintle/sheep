...
ll exp(ll n,ll k,ll m){ll r=1,a=n%m;do{r=k&1?r*a%m:r;a=a*a%m;}while(k/=2);return r;}
ll imod(ll a,ll m){return exp(a,m-2,m)%m;}
ll C(ll n,ll r,ll m){return r?(F[n]*imod(F[r],m)%m*imod(F[n-r],m)%m)%m:1;}
void facto(ll n,ll m){F[0]=1;F1(n,i)F[i]=(F[i-1]*i)%m;}
void sieve(ll n){for(ll i=4;i<=n;i+=2)state[i]=0;for(ll i=3;i<=n;i+=2)for(ll j=i*i;j<=n;j+=i)state[j]=0;}
...
long long chinese_remainder_theorem(vector<Congruence> const& congruences) {
    ...
    lll solution = 0;
    for (auto const& congruence : congruences) {
        lll a_i = congruence.a;
        lll M_i = M / congruence.m;
        lll N_i = imod(M_i, congruence.m);
        solution = (solution + a_i * M_i % M * N_i) % M;
    }
    return solution;
}

vl pre[4000000], divi {3, 11, 101, 3000301};
...
void solve() {
	...
	cin >> n >> _ >> r;
	n--; r--;
	if(r > n) return void(cout << 0);
	vector<Congruence> cg;
	for(auto p: divi) {
		ll A = factmod(n, p), B = factmod(r, p), C = factmod(n - r, p), Pn = multiplicity_factorial(n, p), Pr = multiplicity_factorial(r, p), Pnr = multiplicity_factorial(n - r, p);
		ll rem = A * imod(B, p) % p * imod(C, p) % p * exp(p, Pn - Pr - Pnr, p) % p;
		cg.push_back(Congruence {rem, p});
	}
	ll ans = chinese_remainder_theorem(cg);
	cout << ans * 2 % 10000003233;
}
