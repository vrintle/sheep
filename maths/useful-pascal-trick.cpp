...
ll C(ll n, ll r) {
	if(n < r || r < 0) return 0;
	ll num = fac[n], den = ifac[r] * ifac[n - r] % M;
	return num * den % M;
}

// Useful pascal's trick:
/*
 * (n)C(k) = (n-1)C(k) + (n-1)C(k-1)
 *
 * this helps us to simplify, sum_{k=0}^{k=m}{(n)C(k)}
 *
 * sum_{ k=0 }^{ k=m }{ (n)C(k) } = 2 * sum_{ k=0 }^{ k=m }{ (n-1)C(k) } - (n-1)C(m)
 *
 */
...
void solve() {
	...
	// counting seq that goes bad at index i
	vl dp(n);
	dp[0] = 1;
	ans = (ans + dp[0] * pw2[n - 2]) % M;
	for(ll i = 1; i <= n - 2; i++) {
		dp[i] = (dp[i - 1] * 2 - C(i - 1, m - 2) + M) % M;
		debug(i, dp[i], pw2[n - i - 2]);
		ans = (ans + dp[i] * pw2[n - i - 2]) % M;
	}
	cout << ans;
}
