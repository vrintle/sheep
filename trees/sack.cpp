...
void solve() {
	...
	function<void(ll, ll, ll)> dfs = [&](ll i, ll p, ll j) -> void {
		ll mx = 0, big = i, eg = -1;
		for(auto& [e, x]: g[i]) {
			if(e != p) {
				dfs(e, i, x);
				if(sz(cnt[e]) > mx) mx = sz(cnt[e]), big = e, eg = x;
			}
		}
		if(j == -1) return ;
		if(big != i) {
			...
			if(e != p && e != big) {
				for(auto& [k, v]: cnt[e]) {
					if(cnt[i].find(k) == cnt[i].end()) {
						...
						if(cnt[i][k] + v == tot[k]) wgt[j] -= k;
						cnt[i][k] += v;
					}
				}
			}
		}
		if(cnt[i].find(a[i]) == cnt[i].end()) {
			if(1 < tot[a[i]]) wgt[j] += a[i];
			cnt[i][a[i]] += 1;
		} else {
			if(cnt[i][a[i]] + 1 == tot[a[i]]) wgt[j] -= a[i];
			cnt[i][a[i]] += 1;
		}
	};
	F0(n - 1, i) {
