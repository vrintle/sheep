...
class lazy {
	...
	a3 flip(int i, int tl, int tr, int l, int r) {
		if(p[i]) push(i);
		...
		int tm = (tl + tr) >> 1;
		a3 r1 = flip(i * 2, tl, tm, l, r), r2 = flip(i * 2 + 1, tm, tr, l, r);
		return (t[i] = fx(r1, r2));
	}
	...
	a3 sum(int i, int tl, int tr, int l, int r) {
		if(p[i]) push(i);
		if(tr <= l) return ldef;
		if(tl >= r) return rdef;
		if(l <= tl && tr <= r) return t[i];
		int tm = (tl + tr) >> 1;
		a3 r1 = sum(i * 2, tl, tm, l, r), r2 = sum(i * 2 + 1, tm, tr, l, r);
		return fx(r1, r2);
	}
	long long sum(int l, int r) {
		return sum(1, 0, n, l, r)[2];
	}
};
