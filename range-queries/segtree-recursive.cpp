...
class lazy {
	...
	node fx(node x, node y) {
		return node {
			max({x.r, y.r, (y.mx == inf || x.mn == -inf) ? 0 : (y.mx - x.mn)}),
			max((x.mx == inf ? -inf : x.mx), (y.mx == inf ? -inf : y.mx)),
			min((x.mn == -inf ? inf : x.mn), (y.mn == -inf ? inf : y.mn))
		};
	}
	void push(int i) {
...
void solve() {
	...
	F0(m, i) b.pb(b[i]);
	F0(m, i) b.pb(b[i]);
	st.build(sz(b), node{0, inf, inf}, node{0, -inf, -inf}, [&](auto x, auto y) {
		return node {
			max({x.r, y.r, (y.mx == inf || x.mn == -inf) ? 0 : (y.mx - x.mn)}),
			max((x.mx == inf ? -inf : x.mx), (y.mx == inf ? -inf : y.mx)),
			min((x.mn == -inf ? inf : x.mn), (y.mn == -inf ? inf : y.mn))
