...
class sparse {
	...
	vector<vector<T>> t;
	int n, m;
	void build(int _n, vector<T> &a, int _m = 20) {
		n = _n, m = _m;
		t.assign(m + 1, vector<T>(n + 1));
	...
	T query(int l, int r) {
		int j = level(r - l + 1);
		// min of seg with length 2^j starting at l and ending at r
...
void solve() {
	...
	for(ll i = 1; i <= n; i++) {
		for(ll j = i; j <= n; j++) {
			ans += max(pfx[i - 1], pfx[j]) - Min.query(i - 1, j);
		}
	}
	cout << ans;
}
