...
const int N = 1e5 + 7, B = 350;
vpl g[N];
vl q[N];
ll tin[N], tout[N], a[N], et[N * 2], idx[N], ans[N], cnt[B], fr[N];
...
void solve() {
	...
	dfs(0, -1);
	for(ll i = 1; i < n; i++) {
		q[i] = {tin[i], tout[i] + 1, idx[i]};
	}
	sort(q + 1, q + n, [&](auto& x, auto& y) {
	...
	ll l = 0, r = 0; // l is 0-indexed, r is 1-indexed: basically [l, r)
	for(ll i = 1; i < n; i++) {
		while(r > q[i][1]) {
			r--;
			remove(r);
		...
		for(ll j = B - 1; j >= 0; j--) {
			if(cnt[j]) {
				for(ll k = j * B; k < (j + 1) * B; k++) {
					if(fr[k] >= 4) {
						ans[q[i][2]] = max(ans[q[i][2]], k);
					}
				}
				break;
			}
	...
	l = 0, r = n * 2; // l is 0-indexed, r is 1-indexed: basically [l, r)
	for(ll i = 1; i < n; i++) {
		while(r > q[i][1]) {
			r--;
			add(r);
