...
void solve() {
	...
	function<vvl(vvl, vvl)> multiply = [&](vvl A, vvl B) -> vvl {
		assert(sz(B) != 0 && sz(A) != 0);
		assert(sz(A[0]) == sz(B)); // required for matrix multiplication
		ll n = sz(A), m = sz(B[0]), z = sz(B);
		vvl ans(n, vl(m));
		for(ll i = 0; i < n; i++) {
			for(ll j = 0; j < m; j++) {
				for(ll k = 0; k < z; k++) {
	...
	vvl I(k, vl(k));
	F0(k, i) I[i][i] = 1;
	function<vvl(vvl, ll)> matrix_expo = [&](vvl A, ll k) -> vvl {
		if(k == 0) return I;
		vvl B = matrix_expo(A, k / 2);
		vvl B_sq = multiply(B, B);
		return multiply(B_sq, k % 2 ? A : I);
	};
	vvl all_mtx = matrix_expo(mtx, n);
	vvl init(k, vl(1));
	init[0][0] = 1;
	vvl all_ways = multiply(all_mtx, init);
	ll m;
	cin >> m;
	vl a(m);
	F0(m, i) {
	...
	vvl wrong_mtx = matrix_expo(mtx, n);
	vvl wrong_ways = multiply(wrong_mtx, init);
	ll ans = all_ways[0][0] - wrong_ways[0][0];
	cout << (ans + M) % M;
}
